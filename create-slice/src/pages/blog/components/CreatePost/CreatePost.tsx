  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (editingPost) {
      dispatch(
        ...
        .unwrap() // lấy data get được từ createAsyncThunk để xử lý lỗi từ server trả về
        .then(() => {
      ...
      try {
        await dispatch(addPost(formData)).unwrap() // phải sử dụng unwrap() thì khi xảy ra lỗi mới nhảy và catch
        setFormData(inititalState)
        if (errorForm) {
          setErrorForm(null)
        }
    ...
    <form onSubmit={handleSubmit} onReset={handleCancelEditPost}>
      ...
      <div className='mb-6'>
        ...
        <input
          type='datetime-local'
          id='publishDate'
          className={`block w-56 rounded-lg border p-2.5 focus:outline-none ${
            errorForm?.publishDate
              ? 'border-red-500 bg-red-50 text-red-900 placeholder-red-900 focus:border-red-500 focus:ring-red-500'
              : 'border-gray-300 bg-gray-50 text-sm text-gray-900 focus:border-blue-500'
          } focus:ring-blue-500`}
          placeholder='Title'
          required
